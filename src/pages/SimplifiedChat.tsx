...
const SimplifiedChat = () => {
  console.log('SimplifiedChat: Component mounted');
  ...
  useEffect(() => {
    console.log('SimplifiedChat: Initializing Gemini service');
    try {
      const initialMessage = geminiService.getInitialMessage();
      setCurrentDisplayText(initialMessage);
      console.log('SimplifiedChat: Gemini service initialized successfully');
    } catch (error) {
      console.error('SimplifiedChat: Failed to initialize Gemini service:', error);
      setCurrentDisplayText("⚠️ AI service temporarily unavailable. You can still type here!");
    }
  }, []);

  const sendMessage = async () => {
    console.log('SimplifiedChat.sendMessage: Starting with message:', newMessage);
    
    if (!newMessage.trim() || !isUserTurn || isGeneratingResponse) {
      console.log('SimplifiedChat.sendMessage: Blocked - invalid conditions');
      return;
    }

    const userMessageText = newMessage;
    console.log('SimplifiedChat.sendMessage: Processing message:', userMessageText);
    ...
    try {
      console.log('SimplifiedChat.sendMessage: Calling Gemini service...');
      const botResponse = await geminiService.sendMessage(userMessageText);
      console.log('SimplifiedChat.sendMessage: Received bot response');
      
      setCurrentDisplayText(botResponse);
      console.log('SimplifiedChat.sendMessage: Updated display with bot response');
    } catch (error) {
      console.error('SimplifiedChat.sendMessage: Error occurred:', error);
      setCurrentDisplayText("Sorry, I'm having trouble right now. Can you try again? 😊");
    } finally {
      setIsGeneratingResponse(false);
      setIsUserTurn(true);
      console.log('SimplifiedChat.sendMessage: Completed');
    }
  };
          ...
          >
            <div className="flex items-center space-x-3 bg-white rounded-full p-4 shadow-md border border-gray-100">
              <Input
                value={newMessage}
                onChange={(e) => {
                  console.log('SimplifiedChat.Input.onChange: New value:', e.target.value);
                  setNewMessage(e.target.value);
                }}
                onKeyDown={(e) => {
                  console.log('SimplifiedChat.Input.onKeyDown: Key pressed:', e.key);
                  if (e.key === 'Enter' && !e.shiftKey) {
                    console.log('SimplifiedChat.Input.onKeyDown: Enter pressed, calling sendMessage');
                    sendMessage();
                  }
                }}
              ...
              <Button
                onClick={() => {
                  console.log('SimplifiedChat.Button.onClick: Send button clicked');
                  sendMessage();
                }}
                size="sm"
